...
本文介绍如何创建增强型NAT网关实例，为NAT网关添加弹性公网IP和添加标签，以及当不再需要NAT资源时，如何解绑弹性公网IP和删除NAT网关。
...
NAT网关需要绑定公网IP才能正常工作。一个NAT网关最多可绑定20个EIP，其中最多可绑定10个按流量计费的EIP，且每个按流量计费的EIP的最大带宽峰值不能超过200Mbps。您可以通过配额管理申请更多配额。详细信息，请参见[管理配额](/cn.zh-CN/用户指南/通用配置/管理配额.md)。



-   您已经创建了NAT网关和EIP。详细信息，请参见[t2003027.md\#]()和[申请新EIP](/cn.zh-CN/用户指南/申请EIP/申请新EIP.md)。
-   您的账号在2017年11月3日23:59分之前没有NAT带宽包。

    2017年11月3日23:59分前，如果您的账号存在NAT带宽包，则仍需使用NAT带宽包为NAT网关提供公网IP。如需使用EIP绑定NAT网关，相关操作，请参见[为什么在NAT网关控制台不能绑定EIP]()。
...
确保要解绑的EIP没有被任何SNAT或DNAT条目占用。如有占用，请先删除SNAT条目和DNAT条目。详细信息，请参见[删除SNAT条目](/cn.zh-CN/用户指南/SNAT（访问公网服务）/删除SNAT条目.md)和[删除DNAT条目](/cn.zh-CN/用户指南/DNAT（提供公网服务）/删除DNAT条目.md)。
...
-   NAT网关没有绑定EIP，如有绑定请解绑。详细信息，请参见[解绑EIP](/cn.zh-CN/用户指南/NAT网关实例/绑定与解绑EIP/解绑EIP.md)。
-   DNAT列表中没有DNAT条目，如有请删除。详细信息，请参见[删除DNAT条目](/cn.zh-CN/用户指南/DNAT（提供公网服务）/删除DNAT条目.md)。
-   SNAT列表中没有SNAT条目，如有请删除。详细信息，请参见[删除SNAT条目](/cn.zh-CN/用户指南/SNAT（访问公网服务）/删除SNAT条目.md)。
