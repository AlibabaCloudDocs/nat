...
3.  在**创建NAT网关**页面，配置以下购买信息，然后单击**立即购买**。
    ...
    -   **计费类型**
        -   **按使用量计费**：按NAT网关实际使用量收费，更多信息，请参见[按使用量计费](/cn.zh-CN/购买指南/按量付费.mdsection_v5g_sue_5bj)。
        -   **按固定规格计费**：每个计费周期的费用固定，不会因NAT网关的使用量发生变化。选择该计费类型后，需要选择**规格**。更多信息，请参见[按固定规格计费](/cn.zh-CN/购买指南/按量付费.md)。
