...
5.  在**创建DNAT条目**面板，配置DNAT条目参数，然后单击**确定**。
    ...
    |配置|说明|
    |--|--|
    |**公网IP地址**|在下拉列表选择要提供互联网通信的公网IP。 **说明：**
    ...
    |**私网IP地址**|选择要通过DNAT规则进行互联网通信的ECS实例。您可以通过以下两种方式指定目标ECS实例的私网IP：    -   **从ECS或弹性网卡对应IP进行选择**：从ECS实例或弹性网卡列表中选择ECS实例。
    -   **自填**：输入目标ECS实例的私网IP。 |
...
**说明：** 请确保ECS实例的安全组规则允许互联网访问ECS实例，更多信息，请参见[安全组概述](/cn.zh-CN/安全/安全组/安全组概述.md)。
