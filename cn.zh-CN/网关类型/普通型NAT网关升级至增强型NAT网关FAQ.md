...
如果您的ECS实例拥有多个ENI，且其中有ENI绑定了公网地址。当访问ECS实例的流量从绑定了公网地址的ENI进入，从未绑定公网地址的ENI流出时，升级到增强型NAT网关后会导致流量不通。如果您的ECS实例符合这种场景，请您在升级到增强型NAT网关前，调整ECS实例内的路由配置，确保从某一个ENI进入ECS实例的公网流量从同一个ENI流出，具体操作，请参见[配置网卡路由](/cn.zh-CN/网络/弹性网卡/配置弹性网卡.md)。
