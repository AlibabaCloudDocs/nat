...
**Note:** If the prices in the following table are inconsistent with those on the buy page, refer to the prices on the [buy page](https://common-buy-intl.alibabacloud.com/?&commodityCode=nat_gw_intl#/buy).
...
-   Capacity unit fee per hour = Unit price of capacity unit \(CNY/unit\) × Number of capacity units
    ...
    |---------|-------------|-------------------------|----------------------------------------------------------------------------|
    |Number of new connections \(CPS\)|Second|1,000|The system collects all CPS values within a billing cycle and then divides the highest CPS value by the capacity unit coefficient to calculate the number of capacity units. |
